...
void VisibleGameObject::SetRotation(float pitch, float yaw, float roll)
{
...
void VisibleGameObject::SetRotation(float quat[4])
{
    // Conversion de Quaternion a Matriz
...
void VisibleGameObject::DrawProgrammablePipeline()
{
    ...
    for (int i = 0; i < obj.getNumberOfMeshes(); ++i)
    {
        ...
        if (pMaterial->bumpMapFilename.empty())
        {
            ...
            if (MainEngine::_enableBumpMapping) { glUniform1i(glGetUniformLocation(MainEngine::shaders["MainShader"], "bEnableBumpMapping"), 0); }
            ...
            if (MainEngine::_enableBumpMapping)
            {
                glUniform1i(glGetUniformLocation(MainEngine::shaders["MainShader"], "colorMap"), 0);
                glUniform1f(glGetUniformLocation(MainEngine::shaders["MainShader"], "materialAlpha"),
                            _isReflective ? 0.7 : pMaterial->alpha);
            }
        }
        else
        {
            // Normal mapping code path.
            if (MainEngine::_enableBumpMapping) { glUniform1i(glGetUniformLocation(MainEngine::shaders["MainShader"], "bEnableBumpMapping"), 1); }
